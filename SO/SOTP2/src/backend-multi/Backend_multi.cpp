...
int main(int argc, const char* argv[]) {
  ...
  while (true) {
    if ((socketfd_cliente = accept(socket_servidor, (struct sockaddr*) &remoto, (socklen_t*) &socket_size)) == -1)
      ...
      //close(socket_servidor);//PREGUNTAR Q ONDA ESTO!
      sockets.push_back(socketfd_cliente);

      pthread_t p;
      pthreads.push_back(p);
      pthread_create(&(pthreads.back()), NULL, (void*(*)(void*))atendedor_de_jugador, (void*)(&sockets.back()));
      //ANDAA YA LA PARTE DE ACEPTAR MUCHOS BROWSERS!!! FALTA SEMAFOROS! Y Q NO SE ROMPA NADA
    }
  }
  return 0;
}

void* atendedor_de_jugador(void* socket_v) {
  int socket_fd=*((int*)socket_v);
  ...
  while (true) {
    ...
    if (comando == MSG_LETRA) {
      ...
      if (es_ficha_valida_en_palabra(ficha, palabra_actual)) {
        ...
        tllock[ficha.fila*ancho+ficha.columna].wlock();
        ...
        tllock[ficha.fila*ancho+ficha.columna].wunlock();
      } else {
...
void quitar_letras(list<Casillero>& palabra_actual) {
  for (list<Casillero>::const_iterator casillero = palabra_actual.begin(); casillero != palabra_actual.end(); casillero++) {
    tllock[casillero->fila*ancho+casillero->columna].wlock();
    tablero_letras[casillero->fila][casillero->columna] = VACIO;
    tllock[casillero->fila*ancho+casillero->columna].wunlock();
  }
  palabra_actual.clear();
}
