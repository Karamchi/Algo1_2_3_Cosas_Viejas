...
MAX_BALL_SPEED = 2.2; %wont accelerate bast this, dont set too high or bugs.
...
ballPlot2 = []; %main plot, includes ball and walls
polPlotI = [];
polPlotD = [];
...
%called once at start of program
  function createFigure
    ...
    set(paddle2Plot, 'Color', PADDLE_COLOR);
      ...
      ballPlot2 = plot(0,0);
      set(ballPlot2, 'Marker', BALL_SHAPE);
      set(ballPlot2, 'MarkerEdgeColor', BALL_OUTLINE);
      set(ballPlot2, 'MarkerFaceColor', [.8, .8, .0]);
      set(ballPlot2, 'MarkerSize', BALL_MARKER_SIZE);
...
%called from main loop on every frame
  function refreshPlot
    ...
    set(ballPlot2, 'XData', paddle2(1,1), 'YData', yinter);
    set(polPlotI, 'Xdata', ipinterx, 'YData', ipintery);
    set(polPlotD, 'Xdata', dpinterx, 'YData', dpintery);
...
%------------guardaPosiciones------------
    function guardaPosiciones
        posX(size(posX)+1) = ballX;
        posY(size(posY)+1) = ballY;
        posT = 0:COUNTER; % Nuevo codigo JJ. Parametrizacion.
    end



%----------------------MoverPaddleIzq-------------------
    function moverPaddleIzq(type,puntosATomar)
        
        yinter = 0;
        %puntosATomar = 10;
        if(COUNTER<puntosATomar*2)
            return
        end
        
        x = posX(end-puntosATomar:end);
        y = posY(end-puntosATomar:end);
        t = posT(end-puntosATomar:end);
        pos = paddle1(1,1);
         
        [yinter,ipinterx,ipintery] = moverPaddle(t,x,y,pos,GOAL_BOT,GOAL_TOP,type);
        
        paddle2;
        if yinter>mean([paddle1(2,2),paddle1(2,3)])
            paddle1(2,:) = paddle1(2,:) + PADDLE_SPEED; 
        end
...
%----------------------MoverPaddleDer-------------------
    function moverPaddleDer(type,puntosATomar)
        yinter = 0;
        if(COUNTER<puntosATomar*2)
            return
        end
        ...
        [yinter,dpinterx,dpintery] = moverPaddle(t,x,y,pos,GOAL_BOT,GOAL_TOP,type);
         
        paddle2;
        if yinter>mean([paddle2(2,2),paddle2(2,3)])
            paddle2(2,:) = paddle2(2,:) + PADDLE_SPEED; 
        end
        if yinter<=mean([paddle2(2,2),paddle2(2,3)])
            paddle2(2,:) = paddle2(2,:) - PADDLE_SPEED; 
        end
...
%----------------------MoverPaddle----------------------
    function [yval,plotvalsx,plotvalsy] = moverPaddle(t,x,y,pos,lim1,lim2,type)
        if type == 1
            %Lagrange
            [yval, plotvalsx, plotvalsy] = lagrangeResuelto(t,x,y,pos,lim1,lim2);
        elseif type == 2
            %splines
            [yval, plotvalsx, plotvalsy] = splineResuelto(t,x,y,pos,lim1,lim2);
        elseif type == 3
            % CM
            [yval, plotvalsx, plotvalsy] = cmResuelto(t,x,y,pos,lim1,lim2);
        end
    end
%----------------------MAIN SCRIPT----------------------
createFigure;
