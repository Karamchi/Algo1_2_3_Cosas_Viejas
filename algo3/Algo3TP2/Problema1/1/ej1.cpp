...
//CEMENTERIO
		...
		/*matrices[i][0].tiempo = matrices[j][0].tiempo + tiempos[e].bmx;
		if(e<=motos){
			matrices[i][e*fila].tiempo = matrices[j][(e-1)*fila].tiempo + tiempos[e].moto;
		}
		if(e<=buggies){
			matrices[i][e].tiempo = matrices[j][e-1].tiempo + tiempos[e].buggy;
		}
		for (m=1; ((m<e) and (m<=motos)); m++){
			if((matrices[j][(m-1)*fila].tiempo + tiempos[e].moto) <= (matrices[j][m*fila].tiempo + tiempos[e].bmx)){
			 	matrices[i][m*fila].tiempo = matrices[j][(m-1)*fila].tiempo + tiempos[e].moto;
			}else{
				matrices[i][m*fila].tiempo = matrices[j][m*fila].tiempo + tiempos[e].bmx;
			}
		}
		for (b=1; ((b<e) and (b<=buggies)); b++){
			if((matrices[j][b-1].tiempo + tiempos[e].buggy) <= (matrices[j][b].tiempo + tiempos[e].bmx)){
			 	matrices[i][b].tiempo = matrices[j][b-1].tiempo + tiempos[e].buggy;
			}else{
				matrices[i][b].tiempo = matrices[j][b].tiempo + tiempos[e].bmx;
			}
		}*/
...
/*if((e-1)>buggies){
		...
		m = e-b;
		while((m<=motos) and (m<e)){
			if((matrices[j][m*fila+b-1].tiempo + tiempos[e].buggy) <= (matrices[j][(m-1)*fila+b].tiempo + tiempos[e].moto)){
		 		matrices[i][m*fila+b].tiempo = matrices[j][m*fila+b-1].tiempo + tiempos[e].buggy;
			}else{
				matrices[i][m*fila+b].tiempo = matrices[j][(m-1)*fila+b].tiempo + tiempos[e].moto;
			}
			if(b>0){
				b--;
			}
